...
def segment_image(image_hsv):
    # 设定颜色阈值，提取符合颜色的区域
    lower_bound = np.array([0, 0, 0])  # 自定义下限
    upper_bound = np.array([255, 255, 255])  # 自定义上限
...
def detect_ripeness(img_hsv, fruit_type):
    ...
    if fruit_type == "Apple":
        ...
        # 成熟香蕉为黄色
        ripe_lower = np.array([25, 80, 80])   # 宽泛的黄色下限
        ripe_upper = np.array([40, 255, 255]) # 宽泛的黄色上限
        unripe_lower = np.array([35, 50, 50])  # 未成熟颜色下限（绿色）
        unripe_upper = np.array([85, 255, 255])  # 未成熟颜色上限
   ...
   # 找到水果的轮廓
    contours, _ = cv2.findContours(ripe_mask + unripe_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # 计算水果的总区域
    total_fruit_area = 0

    for contour in contours:
        total_fruit_area += cv2.contourArea(contour)

    # 计算成熟和腐烂区域的像素数
    ripe_area = cv2.countNonZero(ripe_mask)
    unripe_area = cv2.countNonZero(unripe_mask)

    print(ripe_area, unripe_area) #测试用
...
if __name__ == '__main__':
    # 读取图像
    image_path = "D:\VS Code\Coding\CV\Group - Assignment\Apple_Notgood.jpg" # 请根据自己的图像路径替换
    segmented_image, edges = preprocess_image(image_path)
    image_hsv = cv2.cvtColor(segmented_image, cv2.COLOR_HSV2BGR)
    # image = cv2.imread(image_path)
    # image_hs = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    is_ripe = detect_ripeness(image_hsv, "Apple")  # 检测成熟  模型训练还没自动检测时，先手动输入
    if is_ripe:
        print("水果成熟")
    else:
        print("水果未成熟")
